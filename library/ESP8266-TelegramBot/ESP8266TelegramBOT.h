...
class TelegramBOT
{
  public:
    TelegramBOT (String, String, String);
  	String message[10][6];  // amount of messages read per time  (update_id, name_id, name, lastname, chat_id, text)
  	void begin(void);
  	void analizeMessages(void);
  	void sendMessage(String chat_id, String text, String reply_markup);
  	void getUpdates(String offset);
		void getAllUpdates_Kalle(bool ClearAllUpdates);
		void analizeMessages_Kalle(void);
