...
String TelegramBOT::connectToTelegram(String command)  {
	...
	IPAddress server(149,154,167,200);//198);
...
/***************************************************************
 * GetUpdates - function to receive all messages from telegram *
 * (Argument to pass: true/false)             *
 ***************************************************************/
void TelegramBOT::getAllUpdates_Kalle(bool ClearAllUpdates)  {
     
    //Serial.println("GET Update Messages ");
    String command="bot"+_token+"/getUpdates?";//retreives all (not allready cleared) messages
    String mess=connectToTelegram(command);       //recieve reply from telegram.org
    // parsing of reply from Telegram into separate received messages
    int i=0; //messages received counter
    if (mess!="") {
			//Serial.println("Sent Update request");
			//Serial.println(offset);
			String a="";
			int ch_count=0;
			String c;
			for (int n=1; n<mess.length()+1; n++) {   //Search for each message start (n=next char index) -borde börja på 20 för att skippa första
					ch_count ++;
					c =  mess.substring(n-1,n); //last char
					//Serial.print(c);
					
					a=a+c; //Creates a string of chars
					if (ch_count>8) { //When wh have reached char 9, start evaluating the last 9 chars to see if it is "update_id"
							if (a.substring(ch_count-9)=="update_id")  { //we found first instance of update_id (every message has one) substring(from)
									//if (i>1) break; //break for loop if more than 2 messages is received!(?)
									 
									message[i][0]=a.substring(0, ch_count-11);//Store last message up til "update_id" (First one is {"ok":true,"result": which will be deleted later)
									a=a.substring(ch_count-11); //remove everything before '{"update_id'
									i++; //nr of messages
									ch_count=11; //Everything before "update_id" is removed from a => we're now at char 11 (after "update_id")
							}
					}
			}
			if (i>0)  { //get the last message 
    ...
    if (i==0) {
        //Serial.println("no new messages");
        //Serial.println();
        message[0][0]="0"; //number of messages
    }
    else {
        message[0][0]=String(i);   //Replaces the "{"ok":true,"result":" with number of messages in the array       
        analizeMessages_Kalle();
				Serial.println();
    ...
    if(ClearAllUpdates==true && i>0)
    {
				Serial.println("All messages cleared!");
				//Serial.println(message[0][1]);
				//delay(500);
        command="bot"+_token+"/getUpdates?offset="+message[0][1];
        mess=connectToTelegram(command);   //Call getUpdates with a high enough update_id to clear all read messages 
    }
		//Serial.println(" ");
} 
...
void TelegramBOT::analizeMessages_Kalle(void){

	//message[i][0] 0-5 => (update_id, name_id, name, lastname, chat_id, text)
	int i;
	for (i=1;i<message[0][0].toInt();i++){ //loop through x nr of messages
	  
		int messageLenght=message[i][0].length();
		//Serial.print(i);
		//Serial.print(": ");
		
		String a=message[i][0]; //Copy message i to string a
		//Serial.println(a);
		int StartPos;
		//loop through current message and loock for json-titles
		for (int m=12; m<messageLenght+1; m++){
			
			//Search for "update_id" pointer start
			if (a.substring(m-12,m)=="\"update_id\":")     { 
				StartPos=m; //character after found string
				while(a.substring(m-1,m)!=","){
					m++;
				}
				message[i][0]=a.substring(StartPos,m-1);
				
			}
			
			//Search for "from" pointer start
			if (a.substring(m-13,m)=="\"from\":{\"id\":")  { 
				StartPos=m+1; //character after found string
				while(a.substring(m-1,m)!=","){
					m++;
				}
				message[i][1]=a.substring(StartPos,m-1);
			}
			
			//Search for "first_name" pointer start
			if (a.substring(m-14,m)=="\"first_name\":\"")  { 
				StartPos=m+1; //character after found string
				while(a.substring(m-1,m)!="\""){
					m++;
				}
				message[i][2]=a.substring(StartPos,m-1);
			}
			
			//Search for "last_name" pointer start
			if (a.substring(m-13,m)=="\"last_name\":\"")   { 
				StartPos=m+1; //character after found string
				while(a.substring(m-1,m)!="\""){
					m++;
				}
				message[i][3]=a.substring(StartPos,m-1);
			}
			
			//Search for "chat" pointer start
			if (a.substring(m-13,m)=="\"chat\":{\"id\":")  { 
				StartPos=m+1; //character after found string
				while(a.substring(m-1,m)!=","){
					m++;
				}
				message[i][4]=a.substring(StartPos,m-1);
			}
			
			//Search for "text" pointer start
			if (a.substring(m-8,m)=="\"text\":\"")         { 
				StartPos=m; //character after found string
				m+=1;
				while(a.substring(m,m+1)!="\""){
					m++;
				}
				message[i][5]=a.substring(StartPos,m);
				//Serial.print(message[i][5]);
			}
		}
	}
	int NextMessageID=message[i-1][0].toInt()+1;
	message[0][1]=NextMessageID;     //Offset after last read message (used to clear all...)
}
